...
def translation(text):
    ...
    result = translator.translate(text, dest='en', src='vi')
    return result.text

def main():
    st.sidebar.title("Chọn loại mô hình")
    page = st.sidebar.selectbox("", ["Văn bản sang hình ảnh", "Hình ảnh sang hình ảnh"])

    st.title('Ứng dụng Web hỗ trợ sử dụng mô hình tạo ảnh')

    col1, col2, col3 = st.columns([2, 3, 2])
    
    if page == "Văn bản sang hình ảnh":
        Text2Img(col1, col2, col3)
    elif page == "Hình ảnh sang hình ảnh":
    ...
    for idx in range(0, len(st.session_state['imgs']), 2):
        ...
        row[0].image(img, caption=f'Ảnh {idx+1}', use_column_width=True)
        
        # Check if there's a second image to display
        if idx + 1 < len(st.session_state['imgs']):
            # Display the second image in the pair
            row[1].image(st.session_state['imgs'][idx + 1], caption=f'Ảnh {idx+2}', use_column_width=True)
...
def Text2Img(col1, col2, col3):
    ...
    # Text input box
    user_input = col1.text_input("Nhập văn bản:")

    # Select models
    model_opts = ["amused/amused-256", "amused/amused-512", "runwayml/stable-diffusion-v1-5"]
    selected_model = col1.selectbox("Chọn mô hình:", model_opts)

    # Number of images input
    num_images_opts = [1, 2, 4]
    num_images = col1.selectbox("Số lượng hình ảnh:", num_images_opts)

    # Image quality input
    image_quality = col1.slider("Chất lượng hình ảnh:", min_value=1, max_value=100, value=50, step=1)
    
    # Generate button
    if col1.button("Tạo ảnh"):
        if user_input:
            user_input = translation(user_input)
            print(user_input)
            with st.spinner('Đang tạo ảnh...'):
                ...
                if len(generated_images) == 1:
                    ...
                    col2.image(generated_images[0], caption=f'Ảnh 1', use_column_width=True)
                    
                    # Offer download button for the single image
                    img_bytes = pil_to_bytes(generated_images[0])
                    col2.download_button(label=f"Tải ảnh", data=img_bytes, file_name=f"generated_image_1.jpg", mime='image/jpg')
                else:
                    ...
                    for idx in range(0, len(generated_images), 2):
                        ...
                        # Display the first image in the pair
                        row[0].image(generated_images[idx], caption=f'Ảnh {idx+1}', use_column_width=True)
                        
                        # Check if there's a second image to display
                        if idx + 1 < len(generated_images):
                            # Display the second image in the pair
                            row[1].image(generated_images[idx + 1], caption=f'Ảnh {idx+2}', use_column_width=True)
                            ...
                            row[0].download_button(label=f"Tải ảnh {idx+1}", data=img_bytes_1, file_name=f"generated_image_{idx+1}.jpg", mime='image/jpg')
                            row[1].download_button(label=f"Tải ảnh {idx+2}", data=img_bytes_2, file_name=f"generated_image_{idx+2}.jpg", mime='image/jpg')
                        # else:
                            # Display placeholder if there's no second image
                            # row[1].info("No second image to display.")
        else:
            col2.warning("Bạn chưa nhập văn bản.")
...
def Img2Img(col1, col2, col3):
    ...
    uploaded_file = col1.file_uploader("Tải ảnh lên:", type=["jpg", "jpeg", "png"])
    ...
    if uploaded_file is not None:
        ...
        col1.image(preview_image, caption="Ảnh hiện tại")
        ...
        if user_input and uploaded_file:
            ...
            with st.spinner('Đang tạo ảnh...'):
                ...
                if len(generated_images) == 1:
                    ...
                    # Offer download button for the single image
                    img_bytes = pil_to_bytes(generated_images[0])
                    col2.download_button(label=f"Tải ảnh 1", data=img_bytes, file_name=f"generated_image_1.jpg", mime='image/jpg')
                else:
                    # Display the generated images in pairs
                    for idx in range(0, len(generated_images), 2):
                        # Create a new row to display two images side by side
            ...
            if not user_input:
                ...
                col2.warning("Bạn chưa tải ảnh lên")
